	...
	<main class="d-flex flex-nowrap">
		...
		<div class="d-flex flex-column flex-shrink-0 p-3 bg-light"
			...
			<ul class="nav nav-pills flex-column mb-auto">
				...
				<hr>
				<li><a href="/build/conmap" class="nav-link active"> <svg
							class="bi pe-none me-2" width="16" height="16">
		...
		<div class="container">
			<!-- 여기부터 작업 공간 시작 -->
			<h1 class="pt-5 text-center">안전지침</h1>
			<h5 class="pt-3 text-center">설정한 지역 : ${location}</h5>
			<h5 class="text-center">설정한 날짜 : ${whatday}</h5>
			<h5 class="text-center">설정한 공정 : ${constr}</h5>
				...
				<div class="col border border-secondary border-3">
					...
					<c:if test="${num == '1'}">
						<p class="text-left" style="font-size: 20px">
							<b>최고기온 : </b>${sessionScope.TMX1}°C</p>
						<p class="text-left" style="font-size: 20px">
							<b>최저기온 : </b>${sessionScope.TMN1}°C</p>
						<p class="text-left" style="font-size: 20px">
							<b>강수확률 : </b>${sessionScope.POP1}%</p>
					</c:if>
					<c:if test="${num == '2'}">
						<p class="text-left" style="font-size: 20px">
							<b>최고기온 : </b>${sessionScope.TMX2}°C</p>
						<p class="text-left" style="font-size: 20px">
							<b>최저기온 : </b>${sessionScope.TMN2}°C</p>
						<p class="text-left" style="font-size: 20px">
							<b>강수확률 : </b>${sessionScope.POP2}%</p>
					</c:if>
					<c:if test="${num == '3'}">
						<p class="text-left" style="font-size: 20px">
							<b>최고기온 : </b>${sessionScope.TMX3}°C</p>
						<p class="text-left" style="font-size: 20px">
							<b>최저기온 : </b>${sessionScope.TMN3}°C</p>
						<p class="text-left" style="font-size: 20px">
							<b>강수확률 : </b>${sessionScope.POP3}%</p>
					</c:if>
				</div>
				<div class="col border border-secondary border-3">
					<p class="text-center" style="font-size: 23px">
						<b>${whatday}일자 시공계획 및 안전지침 작성란</b>
					</p>
					<form action="/build/addconplan">
						<b style="font-size: 15px">메모 : </b>
						<textarea name="dbnote" cols="80" rows="3"></textarea>
						<input type="hidden" name="dbuser" value="${sessionScope.Email}" />
						<input type="hidden" name="dbdate" value="${whatday}" />
						<c:if test="${num == '1'}">
							<input type="hidden" name="dbmaxtem"
								value="${sessionScope.TMX1}°C" />
							<input type="hidden" name="dbmintem"
								value="${sessionScope.TMN1}°C" />
							<input type="hidden" name="dbrain" value="${sessionScope.POP1}%" />
							<c:forEach items="${gpt1}" var="element1">
								<input type="hidden" name="dbgpt" value="${element1}" />
							</c:forEach>

						</c:if>
						<c:if test="${num == '2'}">
							<input type="hidden" name="dbmaxtem"
								value="${sessionScope.TMX2}°C" />
							<input type="hidden" name="dbmintem"
								value="${sessionScope.TMN2}°C" />
							<input type="hidden" name="dbrain" value="${sessionScope.POP2}%" />
							<c:forEach items="${gpt2}" var="element2">
								<input type="hidden" name="dbgpt" value="${element2}" />
							</c:forEach>
						</c:if>
						<c:if test="${num == '3'}">
							<input type="hidden" name="dbmaxtem"
								value="${sessionScope.TMX3}°C" />
							<input type="hidden" name="dbmintem"
								value="${sessionScope.TMN3}°C" />
							<input type="hidden" name="dbrain" value="${sessionScope.POP3}%" />
							<c:forEach items="${gpt3}" var="element3">
								<input type="hidden" name="dbgpt" value="${element3}" />
							</c:forEach>
						</c:if>
						<input type="hidden" name="dblocation" value="${location}" /> <input
							type="hidden" name="dbwork" value="${constr}" /> <input
							class="btn btn-primary mb-2" type="submit" value="계획 저장"
							style="float: right;" />
					</form>
				</div>
			</div>
			...
			<div class="pt-3"></div>
			<div class="border border-secondary border-3 text-left border-3 pt-2">
