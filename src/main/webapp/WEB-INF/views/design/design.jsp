	...
	<main class="d-flex flex-nowrap">
		...
		<div class="container border border-secondary border-3">
			...
			<p class="text-left" style="font-size: 20px;">
				<b>설정한 위치 : </b>${location}</p>
			<p class="text-left" style="font-size: 20px;">
				<b>${location}의 평균 기상 데이터 : </b><br />${feature}</p>
			<form class="pt-4 text-center" action="/build/designGo" method="get">
				<input type="hidden" name="feature" value="${feature}" /> <input
					type="hidden" name="location" value="${location}" /> <input
					type="submit" class="btn btn-primary" value="설계 지침보기" />
			</form>
			...
			<div class="col border border-secondary border-3"
				...
				<p class="text-center" style="font-size: 23px">
					<b>${whatday}일자 설계지침 및 계획 작성란</b>
				</p>
				<form action="/build/adddeplan">
					<b style="font-size: 15px">메모 : </b><br />
					<textarea name="dbnote" cols="165" rows="3"></textarea>
					<br /> <input type="hidden" name="dbuser"
						value="${sessionScope.Email}" /> <input type="hidden"
						name="dbaverweather" value="${feature}" /> <input type="hidden"
						name="dblocation" value="${location}" />
					...
					<input class="btn btn-primary" type="submit" value="계획 저장"
						style="float: right;" />
				</form>
			</div>
