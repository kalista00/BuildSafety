	...
	<main class="d-flex flex-nowrap">
		...
		<div class="d-flex flex-column flex-shrink-0 p-3 bg-light"
			...
			<ul class="nav nav-pills flex-column mb-auto">
				...
				<li><c:if test="${plantype == 'construction'}">
						<a href="/build/myplan?plantype=construction"
							class="nav-link active"> <svg class="bi pe-none me-2"
								width="16" height="16">
								<use xlink:href="#plan" /></svg> 나의 계획 현황
						</a>
					</c:if> <c:if test="${plantype == 'design'}">
						<a href="/build/myplan?plantype=design" class="nav-link active">
		...
		<div class="container">
			<!-- 여기부터 작업 공간 시작 -->
			<h4 class="text-center">희망하는 계획을 선택하세요</h4>
			<div class="d-flex justify-content-center">

				<button class="btn btn-primary"
					onclick="location='/build/conplanlist'">시공 계획</button>
				&nbsp;
				<button class="btn btn-primary"
					onclick="location='/build/deplanlist'">설계 계획</button>
			</div>
			<br />



			<table class="table" border="1" width="1200" cellpadding="0"
				cellspacing="0" align="center">

				<c:if test="${plantype == 'construction'}">
					<h1 class="text-center mb-3">나의 시공 계획서</h1>
					<thead class="table-dark">
						<tr>
							<td align="center">진행 일정</td>
							<td align="center">설정 지역</td>
							<td align="center">공정</td>
							<td align="center">계획 진행</td>
							<td align="center">최고 기온</td>
							<td align="center">최저 기온</td>
							<td align="center">강수 확률</td>
							<td align="center">메모</td>
						</tr>
					</thead>
				</c:if>
				<c:if test="${plantype == 'design'}">
					<h1 class="text-center mb-3">나의 설계 계획서</h1>
					<thead class="table-dark">
						<tr>
							<td align="center">설정 지역</td>
							<td align="center">지역 평균 기상데이터</td>
							<td align="center">메모</td>
						</tr>
					</thead>
				</c:if>
				...
				<c:if test="${list == null}">
					<tr>
						<td align="center" colspan="4">작성한 계획서가 없습니다.</td>
					</tr>
				</c:if>
				...
				<c:if test="${list != null}">
					<c:forEach var="info" items="${list}">
						...
						<c:if test="${plantype == 'construction'}">
							...
							<tr
								onclick="location='/build/plancontent?dbnum=${info.dbnum}&plantype=${plantype}'">
								<td align="center">${info.dbdate}</td>
								<td align="center">${info.dblocation}</td>
								<td align="center">${info.dbwork}</td>
								<td align="center">${info.dbresult}</td>
								<td align="center">${info.dbmaxtem}</td>
								<td align="center">${info.dbmintem}</td>
								<td align="center">${info.dbrain}</td>
								<td align="center">${info.dbnote}</td>
							</tr>
						...
						<c:if test="${plantype == 'design'}">
							<tr
								onclick="location='/build/plancontent?dbnum=${info.dbnum}&plantype=${plantype}'">
								<td align="center">${info.dblocation}</td>
								<td align="center">${info.dbaverweather}</td>
								<td align="center">${info.dbnote}</td>
							</tr>
						</c:if>
					...
					<ul class="pagination">
						<c:if test="${startPage > 10}">
							<li class="page-item"><a class="page-link"
								href="/build/myplan?pageNum=${startPage - 10}&plantype=${plantype}">이전</a></li>
						</c:if>
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							<li class="page-item"><a class="page-link"
								href="/build/myplan?pageNum=${i}&plantype=${plantype}">${i}</a></li>
						</c:forEach>
						<c:if test="${endPage < pageCount}">
							<li class="page-item"><a class="page-link"
								href="/build/myplan?pageNum=${startPage + 10}&plantype=${plantype}">다음</a></li>
						</c:if>
					</ul>
				</nav>
