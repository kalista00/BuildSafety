	...
	<main class="d-flex flex-nowrap">
		...
		<div class="container">
			...
			<c:if test="${plantype == 'construction'}">
				<h1 class="text-center">${dto.dbdate}일자 계획 수정</h1>
				<form action="/build/conupdatepro">
					<input type="hidden" name="dbnum" value="${dto.dbnum}" />
					<table class="table">
						<tbody>
							<tr>
								<td><strong>작업 지역</strong></td>
								<td>${dto.dblocation}</td>
							</tr>
							<tr>
								<td><strong>시공 작업</strong></td>
								<td>${dto.dbwork}</td>
							</tr>
							<tr>
								<td><strong>계획 결과</strong></td>
								<td><select class="form-select" name="dbresult">
										<option value="미완료">미완료</option>
										<option value="완료">완료</option>
								</select></td>
							</tr>
							<tr>
								<td><strong>최고 기온</strong></td>
								<td>${dto.dbmaxtem}</td>
							</tr>
							<tr>
								<td><strong>최저 기온</strong></td>
								<td>${dto.dbmintem}</td>
							</tr>
							<tr>
								<td><strong>강수량</strong></td>
								<td>${dto.dbrain}</td>
							</tr>
							<tr>
								<td><strong>안전지침</strong></td>
								<td><c:forEach items="${dto.dbgpt}" var="element">
  ${element}<br />
									</c:forEach></td>
							</tr>
							<tr>
								<td><strong>메모</strong></td>
								<td><textarea class="form-control" name="dbnote" cols="100"
										rows="5">${dto.dbnote}</textarea></td>
							</tr>
						</tbody>
					</table>
					<div class="d-flex justify-content-center">
						<button class="btn btn-primary"
							onclick="location='/build/conplanlist'">돌아가기</button>
			...
			<c:if test="${plantype == 'design'}">
				<h1 class="text-center">[${dto.dblocation}] 지역의 설계 계획 수정 페이지</h1>
				<form action="/build/deupdatepro">
					<input type="hidden" name="dbnum" value="${dto.dbnum}" />
					<table class="table">
						<tbody>
							<tr>
								<td><strong>지역 평균 기상 데이터</strong></td>
								<td>${dto.dbaverweather}</td>
							</tr>
							<tr>
								<td><strong>설계 지침</strong></td>
								<td><c:forEach items="${dto.dbgpt}" var="element">
  ${element}<br />
									</c:forEach></td>
							</tr>
					...
					<div class="d-flex justify-content-center">
						<button class="btn btn-primary"
							onclick="location='/build/deplanlist'">돌아가기</button>
						&nbsp; <input class="btn btn-success" type="reset" value="초기화" />&nbsp;
						<input class="btn btn-warning" type="submit" value="수정하기" />
					</div>
				</form>
